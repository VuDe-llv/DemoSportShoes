...
if (isset($_GET['sort'])) {
    ...
    $sql_pro = "SELECT * FROM tbl_sanpham WHERE loaisp IN ('Giày cỏ tự nhiên', 'Giày cỏ nhân tạo', 'Giày futsal') AND hangsp IN ('Nike', 'Adidas', 'Puma') $orderBy";
} else {
    // Mặc định sắp xếp theo Mới nhất
    $sql_pro = "SELECT * FROM tbl_sanpham WHERE loaisp IN ('Giày cỏ tự nhiên', 'Giày cỏ nhân tạo', 'Giày futsal') AND hangsp IN ('Nike', 'Adidas', 'Puma') ORDER BY id_sanpham DESC";
}
